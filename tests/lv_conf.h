/**
 * @file lv_conf.h
 * LVGL configuration for the helix-xml test suite.
 *
 * Derived from LVGL v9.5.0's `lv_conf_template.h`, with the switches the XML
 * engine needs turned on.
 *
 * ---------------------------------------------------------------------------
 * THIS CONFIG IS DELIBERATELY INDEPENDENT OF HelixScreen's `lv_conf.h`.
 *
 * helix-xml is its own library with its own repo; its tests must not silently
 * inherit whatever the consuming application happens to configure.  Do not
 * "sync" this file with HelixScreen's - if a test needs a different LVGL
 * option, change it here and say why.
 *
 * CONSEQUENCE FOR TEST AUTHORS: no test in this suite may assert on measured
 * pixel geometry or on font metrics.  Colour depth, the default font, the
 * enabled widget set and the theme here are all chosen for the test build, and
 * are not the values any real device runs.  Assert on structure (widget exists,
 * parent/child relationships, names, flags, states, style property values you
 * set yourself) - never on `lv_obj_get_width()`, text width, or line height.
 * ---------------------------------------------------------------------------
 *
...
/* clang-format off */
#if 1 /* Enabled: this is a real configuration, not a template. */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #define LV_MEM_SIZE (4 * 1024 * 1024U)  /**< [bytes] Generous: tests are not memory constrained,
                                              *   and running out would trip LV_USE_ASSERT_MALLOC. */
...
#define LV_ASSERT_HANDLER abort();      /**< Abort, so a tripped assert kills the test process instead
                                          *   of spinning forever in the upstream `while(1);` default. */
 ...
 *  Kept at 1 for the test suite: `lv_xml_atof` / `lv_xml_atof_split` are
 *  compiled only under this guard, and float subjects are one of helix-xml's
 *  documented extensions over upstream LVGL, so they have to be covered. */
...
#if LV_USE_FS_STDIO
    #define LV_FS_STDIO_LETTER 'A'      /**< Driver-identifier letter. Tests load fixtures as "A:<path>". */
...
/** Enable the Helix XML engine (lib/helix-xml).
 *
 *  LVGL 9.5 removed `src/xml` from core, so this switch does not exist in
 *  upstream's `lv_conf_template.h`.  Every `.c` under `helix-xml/src/xml/`,
 *  `helix-xml/src/xml/parsers/` and the vendored expat in
 *  `helix-xml/src/libs/expat/` is wrapped in `#if LV_USE_XML`, so setting this
 *  to 0 collapses the whole library to empty translation units.
 *
 *  Requires LV_USE_OBJ_NAME and LV_USE_OBSERVER (lv_xml.c #errors without them).
 *  LV_USE_TRANSLATION additionally gates lv_xml_translation.c, and LV_USE_TEST
 *  gates lv_xml_test.c. */
#define LV_USE_XML 1
