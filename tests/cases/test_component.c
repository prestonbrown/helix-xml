 ...
 * Log capture
 *
 * Every resolution failure in this file is reported ONLY as a LV_LOG_WARN - the
 * attribute is dropped and the widget silently keeps its default, which is
 * indistinguishable from "the XML never mentioned it". So the log line is the
 * assertion. lv_log_register_print_cb takes precedence over LV_LOG_PRINTF, so
 * installing one both captures the message and keeps the runner output clean.
 *--------------------------------------------------------------------------*/
